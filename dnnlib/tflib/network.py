...
class Network:
    ...
    def run(self,
        ...
        # key = dict(input_transform=input_transform, output_transform=output_transform, num_cpus=num_cpus, dynamic_kwargs=dynamic_kwargs)
        def unwind_key(obj):
            ...
            with tfutil.absolute_name_scope(self.scope + "/_Run"), tf.control_dependencies(None):
                if custom_inputs is not None:
                    ...
                    with tf.device("/cpu:0"):
                        ...
                        in_split = list(zip(*[tf.split(x, num_gpus) for x in in_expr]))
                        # in_split = list(zip(*[tf.split(x, num_cpus) for x in in_expr]))
